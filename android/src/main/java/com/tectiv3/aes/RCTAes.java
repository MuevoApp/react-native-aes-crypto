...
public class RCTAes extends ReactContextBaseJavaModule {
    ...
    {
        byte[] keyData = hexToBytes(key);
        byte[] contentData = hexToBytes(text);
        Mac sha_HMAC = Mac.getInstance(algorithm);
        SecretKey secret_key = new SecretKeySpec(keyData, algorithm);
        sha_HMAC.init(secret_key);

       byte[] hash = sha_HMAC.doFinal(contentData);
